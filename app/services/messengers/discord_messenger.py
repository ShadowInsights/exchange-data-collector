...
def _generate_message(message: BaseMessage) -> DiscordEmbed:
    embed = DiscordEmbed(title=message.title, description=message.description, color="03b2f8")
    embed.set_timestamp()
...
class DiscordMessenger(BaseMessenger):
    ...
    def __init__(self):
        self.webhooks = [DiscordWebhook(webhook_url, rate_limit_retry=True, username="Anomaly Alerting") for webhook_url in settings.DISCORD_WEBHOOKS.split(",")]
